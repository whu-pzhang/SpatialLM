...
def visualize_wireframe(annos):
    ...
    for cuboid in annos["cuboids"]:
        for planeID in cuboid["planeID"]:
            cuboid_lineID = np.where(np.array(annos["planeLineMatrix"][planeID]))[
                0
            ].tolist()
    ...
    connected_colors = np.repeat(
        colormap[0].reshape(1, 3), len(connected_junctions), axis=0
    )
...
def visualize_plane(annos, args, eps=0.9):
    """
    可视化平面结构，根据给定的注释信息和参数设置，生成3D平面网格并进行可视化。

    参数:
        annos (dict): 包含场景几何与语义信息的字典，包括junctions、planes、semantics等字段。
        args (argparse.Namespace): 命令行参数对象，包含color等配置项。
        eps (float, optional): 法向量判断阈值，默认为0.9。

    返回:
        None: 该函数不返回任何值，直接调用可视化接口显示结果。
    """
    """visualize plane"""
    colormap = np.array(colormap_255) / 255
    junctions = [item["coordinate"] for item in annos["junctions"]]

    # 如果使用曼哈顿世界颜色方案，则构建平面ID到曼哈顿类别的映射
    ...
    for semantic in annos["semantics"]:
        for planeID in semantic["planeID"]:
            ...
            vertices, faces = clip_polygon(
                polygon, vertices_holes, junctions, plane_anno
            )
            polygons.append(
                [
                    vertices,
    ...
    # 构建每个平面的可视化网格对象，并按规则上色
    plane_set = []
    for i, (vertices, faces, planeID, normal, plane_type, semantic_type) in enumerate(
        polygons
    ):
...
def plot_floorplan(annos, polygons):
    ...
    for polygon, poly_type in polygons:
        ...
        if poly_type == "outwall":
            ...
            patch = PolygonPatch(
                polygon, facecolor=semantics_cmap[poly_type], alpha=0.5
            )
        ax.add_patch(patch)
...
def parse_args():
    ...
    parser.add_argument(
        "--type", choices=("floorplan", "wireframe", "plane"), default="plane", type=str
    )
    parser.add_argument(
        "--color", choices=["normal", "manhattan"], default="normal", type=str
    )
...
def main():
    ...
    with open(
        os.path.join(args.path, f"scene_{args.scene:05d}", "annotation_3d.json")
    ) as file:
