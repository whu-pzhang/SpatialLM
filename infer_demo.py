...
if __name__ == "__main__":
    ...
    for point_cloud_file in tqdm(point_cloud_files):
        ...
        if os.path.splitext(args.output)[-1]:
            ...
            txt2dxf(input_file=args.output, output_file=dxf_2d_file, verbose=True)
            # TODO: convert txt to 3D dxf
            ...
            txt2dxf(
                input_file=output_filepath, output_file=str(dxf_2d_file), verbose=True
            )
            #
            dxf_3d_file = Path(output_filepath).joinpath(f"{stem}_3d.dxf")
            converter = Txt2DxfConverter(
